...
#if (BT_FW_DEBUG == TRUE)
        CASE_RETURN_STR(XR_DEBUG_EVENT)
#endif
...
#if (BT_FW_DEBUG == TRUE)
    BTVNDDBG("init firmware debug pthread");
    init_pthread();
...
static int op(bt_vendor_opcode_t opcode, void *param)
{
    ...
    switch(op)
    {
       ...
       case XR_DEBUG_EVENT:
           {
              hci_xr_msg_cback(param);
           }
           break;
#endif
#if 0
       case BT_VND_OP_A2DP_COEX_OPT:
           {
              uint8_t* a2dp_coex_cmd = (uint8_t*)param;
              if (*a2dp_coex_cmd == A2DP_COEX_START)
              {
                hw_a2dp_coex_start(1);
              }
              else
              {
                hw_a2dp_coex_start(0);
              }
           }
           break;
#endif
