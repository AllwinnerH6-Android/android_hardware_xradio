...
void userial_vendor_set_baud(uint8_t userial_baud)
{
    ...
    ALOGI("userial_vendor_set_baud start.");
    ...
    if(tcsetattr(vnd_userial.fd, TCSADRAIN, &vnd_userial.termios)<0)
        ALOGE("tcsetattr fail ");

    tcflush(vnd_userial.fd, TCIFLUSH);

    ALOGI("userial_vendor_set_baud end.");
}
...
void userial_vendor_set_hw_fctrl(uint8_t hw_fctrl)
{
    ...
    if(hw_fctrl)
    {
        ...
        if(vnd_userial.termios.c_cflag & CRTSCTS)
        {
